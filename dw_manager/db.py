...
def seed_catalogs_from_registros(conn: sqlite3.Connection, df: pd.DataFrame) -> dict[str, int]:
    ...
    if all(c in df.columns for c in ["pais", "cliente", "proyecto"]):
        ...
        # Fechas de proyecto/hito derivadas de ClickUp.
        # Esto permite que las tarifas históricas se alineen con el inicio real del proyecto
        # sin obligar al usuario a seleccionar fechas manualmente en pantalla.
        if "fecha_referencia" in df.columns:
            tmp = df.copy()
            tmp["_fecha_ref"] = pd.to_datetime(tmp["fecha_referencia"], errors="coerce")
            tmp = tmp.dropna(subset=["_fecha_ref"])
            if not tmp.empty:
                for _, g in tmp.groupby(["pais", "cliente", "proyecto"], dropna=False):
                    pais = str(g["pais"].iloc[0]).strip()
                    cliente = str(g["cliente"].iloc[0]).strip()
                    proyecto = str(g["proyecto"].iloc[0]).strip()
                    if not pais or not cliente or not proyecto:
                        continue
                    fecha_inicio = g["_fecha_ref"].min().date().isoformat()
                    fecha_fin = g["_fecha_ref"].max().date().isoformat()
                    conn.execute(
                        """
                        UPDATE catalogo_proyectos
                        SET fecha_inicio = CASE WHEN fecha_inicio IS NULL OR date(?) < date(fecha_inicio) THEN ? ELSE fecha_inicio END,
                            fecha_fin = CASE WHEN fecha_fin IS NULL OR date(?) > date(fecha_fin) THEN ? ELSE fecha_fin END
                        WHERE pais_norm=? AND cliente_norm=? AND proyecto_norm=?
                        """,
                        (fecha_inicio, fecha_inicio, fecha_fin, fecha_fin, normalize_key(pais), normalize_key(cliente), normalize_key(proyecto)),
                    )
                if "hito_facturable" in tmp.columns:
                    for _, g in tmp.groupby(["pais", "cliente", "proyecto", "hito_facturable"], dropna=False):
                        pais = str(g["pais"].iloc[0]).strip()
                        cliente = str(g["cliente"].iloc[0]).strip()
                        proyecto = str(g["proyecto"].iloc[0]).strip()
                        hito = normalize_hito(g["hito_facturable"].iloc[0])
                        pid = upsert_proyecto(conn, pais, cliente, proyecto)
                        if not pid:
                            continue
                        fecha_inicio = g["_fecha_ref"].min().date().isoformat()
                        fecha_fin = g["_fecha_ref"].max().date().isoformat()
                        conn.execute(
                            """
                            UPDATE catalogo_hitos
                            SET fecha_inicio = CASE WHEN fecha_inicio IS NULL OR date(?) < date(fecha_inicio) THEN ? ELSE fecha_inicio END,
                                fecha_fin = CASE WHEN fecha_fin IS NULL OR date(?) > date(fecha_fin) THEN ? ELSE fecha_fin END
                            WHERE id_proyecto_catalogo=? AND hito_norm=?
                            """,
                            (fecha_inicio, fecha_inicio, fecha_fin, fecha_fin, pid, normalize_key(hito)),
                        )
                conn.commit()
