...
def apply_tariffs(
    ...
    Cumplimiento en horas = Horas registradas / Horas estimadas
...
def build_detail(df: pd.DataFrame, mode: str = "general") -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    group_cols = ["pais", "cliente", "proyecto", "hito_facturable", "assignee", "rol_estimado", "rol_asignado"]
    agg = df.groupby(group_cols, dropna=False).agg(
    ...
    if mode == "servicios":
        ordered = [
            "pais", "cliente", "proyecto", "hito_facturable", "persona", "rol_estimado", "rol_asignado",
    ...
    return agg[[c for c in ordered if c in agg.columns]].sort_values(["pais", "cliente", "proyecto", "hito_facturable", "persona"])
